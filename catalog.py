...
def garage_items(garage_id):
	...
	items = session.query(Car_Item).filter_by(garage_id=garage.id).all()


	return render_template("garage.html", garage=garage, items=items)
...
def delete_car(category_id, car_id):
